...
void main (void) {
    ...
    vec3 colors[12];
    colors[0] = rgb_normalize(309, 58, 107); // red
    colors[1] = rgb_normalize(380, 100, 203); // red-purple
    colors[2] = rgb_normalize(302, 100, 100); // violet
    colors[3] = rgb_normalize(170, 78, 148); // violet-blue
    colors[4] = rgb_normalize(163, 111, 171); // blue
    colors[5] = rgb_normalize(167, 188, 183); // blue-green
    colors[6] = rgb_normalize(262, 140, 195); // green
    colors[7] = rgb_normalize(200, 186,	168); // green-yellow
    colors[8] = rgb_normalize(141, 229, 76); // yellow
    colors[9] = rgb_normalize(544, 200,	170); // yellow-orange
    colors[10] = rgb_normalize(327,	147, 160); // orange
    colors[11] = rgb_normalize(300,	100, 103); // orange-red
    ...
    if( column == 0.0){
      ...
      // first column: blend between purple & yellow (steps)
    	vec3 yellow = calculateHueLerp(colors, 0.562); 
